...
import com.blackduck.protecode.sc.jenkins.types.BuildVerdict;
import com.blackduck.protecode.sc.jenkins.types.FileResult;
import com.blackduck.protecode.sc.jenkins.utils.*;
...
public class ProtecodeScPlugin extends Builder implements SimpleBuildStep {
  ...
  public boolean doPerform(Run<?, ?> run, FilePath workspace)
    ...
    // If we're using a Black Duck hosted protecode instance, we will use zipping in cases where there are more
    ...
    if (this.dontZipFiles) {
      console.log("'Dont zip' is chosen, but since this build is done against a Black Duck hosted "
